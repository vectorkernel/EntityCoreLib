...
void Application::OnLeftButtonDownClient(int x, int y)
{
    ...
    if (m_state.tool == InteractionState::Tool::Erase && m_state.waitingForSecondClick)
    {
        ...
        // ScreenToWorld takes integer client pixels.
        glm::vec2 w00 = m_pz.ScreenToWorld(x0, y0);
        glm::vec2 w10 = m_pz.ScreenToWorld(x1, y0);
        glm::vec2 w01 = m_pz.ScreenToWorld(x0, y1);
        glm::vec2 w11 = m_pz.ScreenToWorld(x1, y1);
        ...
        for (auto& e : m_book->entities)
        {
            ...
            if (!crossing)
            {
                bool aIn = (a.x >= mn.x && a.x <= mx.x && a.y >= mn.y && a.y <= mx.y);
                bool bIn = (b.x >= mn.x && b.x <= mx.x && b.y >= mn.y && b.y <= mx.y);
                hit = aIn && bIn;
            }
            else
            {
        ...
        // Back to ERASE idle appearance (picker box only)
        m_crosshairs.SetMode(RenderCore::CrosshairsMode::PickerOnly);

        return;
    }

    // -------------------------------------------------
    // Pick-box selection (GripsSelection + Erase)
    // -------------------------------------------------
    const int sidePx = m_crosshairs.GetPickBoxSizePx();
    const int halfPx = std::max(1, sidePx / 2);
    ...
    {
        const bool changed = AppCore::ApplyGripsFromAabb(*m_book, m_state,
            pb.worldMin, pb.worldMax,
    ...
    // -------------------------------------------------
    // Otherwise: regular selection into EntityBook (used by ERASE)
    // -------------------------------------------------
    bool any = false;
    for (auto& e : m_book->entities)
    {
...
void Application::OnKeyDown(WPARAM key, bool /*ctrlDown*/)
{
    ...
    if (key == 'E')
    {
        ...
        if (m_state.tool == InteractionState::Tool::Erase)
            ...
            m_crosshairs.SetMode(RenderCore::CrosshairsMode::GripsSelection); // grips mode
...
{
    OnKeyDown(VK_ESCAPE, false);
}

void Application::CommitErase()
{
    // In ERASE this commits and returns to grips mode
    OnKeyDown(VK_RETURN, false);
}

void Application::BeginWindowSelection(bool crossing)
{
    OnKeyDown(crossing ? 'C' : 'W', false);
}
