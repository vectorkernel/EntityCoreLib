...
void Application::OnLeftButtonDownClient(int x, int y)
{
    ...
    if (!m_lineCommandActive)
        return;

    const glm::vec2 w = m_pz.ScreenToWorld(x, y);
    const glm::vec3 p(w.x, w.y, 0.0f);
    ...
    // End the LINE command after one segment for now.
    m_lineCommandActive = false;
...
// -------------------------------------------------
void Application::StartLineCommand()
{
    if (!m_book) return;

    m_lineCommandActive = true;
    m_lineHasFirst = false;
    m_jigDirty = true;

    // LINE uses point-pick cursor.
    m_crosshairs.SetMode(RenderCore::CrosshairsMode::PointPick);
...
void Application::CancelCommand()
{
    ...
    // Keep cursor in point-pick for LINE.
    m_crosshairs.SetMode(RenderCore::CrosshairsMode::PointPick);

    // Don't clear the drawing; just cancel the jig.
    m_state.requestRedraw = true;
}
...
void Application::OnMouseMoveClient(int x, int y)
{
    ...
    if (m_lineCommandActive && m_lineHasFirst)
    {
        const glm::vec2 w = m_pz.ScreenToWorld(x, y);
...
void Application::Render(float /*inAspectRatio*/)
{
    ...
    if (m_lineCommandActive && m_lineHasFirst && m_jigDirty)
    {
        Vertex v[2]{};
        v[0].pos = m_lineP0;
        v[1].pos = m_lineP1;
